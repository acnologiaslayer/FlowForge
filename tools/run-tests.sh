...
javac -d "${OUT_MAIN}" @/tmp/flowforge_main_sources.txt
...
javac -cp "${OUT_MAIN}:${JUNIT_CP}" -d "${OUT_TEST}" @/tmp/flowforge_test_sources.txt

echo "==> Running tests"
java -cp "${OUT_TEST}:${OUT_MAIN}:${JUNIT_CP}" JUnitRunner
